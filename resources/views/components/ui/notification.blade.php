<div x-data="{
    ...
    @if(session('success'))
      triggerNotification('{{ session('success') }}', 'success');
    @elseif(session('error'))
      triggerNotification('{{ session('error') }}', 'error');
    @elseif(session('warning'))
      triggerNotification('{{ session('warning') }}', 'warning');
    @elseif(session('info'))
      triggerNotification('{{ session('info') }}', 'info');
    @endif
  ">
    ...
    <template x-for="notification in notifications" :key="notification.id">
      <div x-show="notification.visible" x-bind="transitionClasses"
        ...
        <template x-if="notification.type !== 'neutral'">
          <div class="flex size-8 flex-none items-center justify-center rounded-full" :class="{
